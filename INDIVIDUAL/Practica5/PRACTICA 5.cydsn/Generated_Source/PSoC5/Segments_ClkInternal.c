/*******************************************************************************
* File Name: Segments_ClkInternal.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_Start
********************************************************************************
*
* Summary:
...
void Segments_ClkInternal_Start(void) 
{
    ...
    Segments_ClkInternal_CLKEN |= Segments_ClkInternal_CLKEN_MASK;
	Segments_ClkInternal_CLKSTBY |= Segments_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Segments_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
void Segments_ClkInternal_Stop(void) 
{
    ...
    Segments_ClkInternal_CLKEN &= (uint8)(~Segments_ClkInternal_CLKEN_MASK);
	Segments_ClkInternal_CLKSTBY &= (uint8)(~Segments_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Segments_ClkInternal_CLKEN & Segments_ClkInternal_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Segments_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Segments_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Segments_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Segments_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Segments_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Segments_ClkInternal_GetSourceRegister();
    uint16 oldDivider = Segments_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Segments_ClkInternal_CLKEN & Segments_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Segments_ClkInternal_DIV_PTR, clkDivider);
                Segments_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Segments_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Segments_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Segments_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Segments_ClkInternal_DIV_PTR, clkDivider);
				Segments_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Segments_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: Segments_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Segments_ClkInternal_MOD_SRC |= modeBitMask & (uint8)Segments_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: Segments_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Segments_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Segments_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: Segments_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Segments_ClkInternal_MOD_SRC & (uint8)(Segments_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: Segments_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Segments_ClkInternal_GetDividerRegister();
    uint8 oldSrc = Segments_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Segments_ClkInternal_MOD_SRC |= CYCLK_SSS;
        Segments_ClkInternal_MOD_SRC =
            (Segments_ClkInternal_MOD_SRC & (uint8)(~Segments_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Segments_ClkInternal_MOD_SRC & Segments_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Segments_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Segments_ClkInternal_PHASE = clkPhase & Segments_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Segments_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Segments_ClkInternal_PHASE & Segments_ClkInternal_PHASE_MASK;
}
